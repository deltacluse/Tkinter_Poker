...
battedMoney = Label(battingFrame, background='red', height=int(setHeight['battingFrame'] / 16 * 0.6),
                    width=int(FullWidth / 7 * 0.2), textvariable=text_battedMoney, foreground='white')
...
battedMoney = Label(moneyFrame, background='red', height=int(setHeight['myFrame'] / 16 * 0.6),
                    width=int(FullWidth / 7 * 0.2), textvariable=text_battingMoney, foreground='white')
...
for i in range(7):
    ...
    img_battedMoney[i] = PIL.ImageTk.PhotoImage(
        img_battedMoney[i].resize((int(setHeight['myFrame'] * 0.75 * 0.84), int(setHeight['myFrame'] * 0.84))))
    card = Label(CardFrame, background='white', height=setHeight['myFrame'] * 0.8,
                 width=setHeight['myFrame'] * 0.75 * 0.8, image=img_battedMoney[i])
    card.place(x=i * (FullWidth * 0.6 / 8) + (FullWidth * 0.3 * 0.25 - setHeight['myFrame'] * 0.75 * 0.4),
               y=setHeight['myFrame'] * 0.1)
